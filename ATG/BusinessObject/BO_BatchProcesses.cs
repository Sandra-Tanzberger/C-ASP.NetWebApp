...
{
    [Serializable]
    public class BO_BatchProcesses : Collection<BO_BatchProcess>, IEnumerable<BO_BatchProcess>, IComparer
	{
         ...
         public class Enumerator : IEnumerator
         {
	        ...
	        private BO_BatchProcesses t;

	        public Enumerator (BO_BatchProcesses t)
	        {
		        this.t = t;
	        }
		...
		public void Sort(string sortingExpression)
		{
			...
			BO_BatchProcess[] elements =new BO_BatchProcess[this.Items.Count];
			this.Items.CopyTo(elements,0);
			Array.Sort(elements,this);
			//clear old order and copy new order here
			this.Items.Clear();
			foreach(BO_BatchProcess item in elements)
				this.Items.Add(item);
		}
		...
		{
			return new BO_BatchProcesses.Enumerator(this);
		}
